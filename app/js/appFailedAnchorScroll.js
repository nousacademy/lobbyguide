...
kioskApp.controller('SwitchEm', function($scope, $http, $location, $anchorScroll) {
    $scope.currentURL = "touchDirectory";
    $scope.groupedDataComp = {};
    $scope.groupedDataIndiv = {};
    var group;
    $http.get('js/data.json').success(function(data) {
        angular.forEach(data.individuals, function(indivValue, indivProp) {
            if (!$scope.groupedDataIndiv[indivValue.name.charAt(0)]) {
                group = [];
                $scope.groupedDataIndiv[indivValue.name.charAt(0)] = group;
            }
            $scope.groupedDataIndiv[indivValue.name.charAt(0)].push(indivValue);
        });
        angular.forEach(data.companies, function(indivValue, indivProp) {
            if (!$scope.groupedDataComp[indivValue.cname.charAt(0)]) {
                group = [];
                $scope.groupedDataComp[indivValue.cname.charAt(0)] = group;
            }
            $scope.groupedDataComp[indivValue.cname.charAt(0)].push(indivValue);
        });
        console.log($scope.groupedDataIndiv);
        console.log($scope.groupedDataComp);
        //console.log($scope.groupedDataIndiv);
    });
